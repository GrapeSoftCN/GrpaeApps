...
public class insServer {
	...
	public String insUpdate(String insid, String insInfo) {
		return model.resultMessage(
				model.update(insid, JSONHelper.string2json(insInfo)), "修改实例成功");
	}
	...
	public String insDeleteBatch(String insInfo) {
		return model.resultMessage(model.delete(insInfo.split(",")),
				"批量删除实例成功");
	}
	...
	public String insSearch(String insInfo) {
		...
		return StringEscapeUtils
				.unescapeJava(model.resultMessage(0, object.toString()));
	}
	...
	public String insPageBy(int idx, int pageSize, String insInfo) {
		object.put("records",
				model.page(idx, pageSize, JSONHelper.string2json(insInfo)));
		return model.resultMessage(0, object.toString());
	}
}
